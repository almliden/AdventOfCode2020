...
namespace AOC_02
{
    class Program
    {
        ...
        {
            string realInput = @"2-4 r: prrmspx
5-6 p: hpzplphxb
5-8 t: ttttbtttttc
1-6 k: kkkkkk
1-3 q: qqqq
...
2-9 c: ccccccccc";
            ...
            int testResult = solver.Solve(testInput);
            int testResultNew = solver.SolveNew(testInput);
            int realResult = solver.Solve(realInput);
            int realResultNew = solver.SolveNew(realInput);

            Console.WriteLine($"testResult: {testResult} realResult: {realResult} testResultNew: {testResultNew} realResultNew: {realResultNew}");
        }
    }
    ...
    class Solver
    {
        ...
        {
            var pwRules = CreateRules(input);
            return ValidateRules(pwRules);
        }

        public int SolveNew(string input)
        {
            var pwRules = CreateRules(input);
            return ValidateRulesNewPolicy(pwRules);
        }

        private List<PasswordRule> CreateRules(string s)
        {
            List<string> rows = new Regex(@"\r\n").Split(s).ToList();
            List<PasswordRule> rules = new List<PasswordRule>();
            foreach (string row in rows)
            {
                string rule = row.Split(":")[0];
                int StartRange = Convert.ToInt32(rule.Split("-")[0]);
                int EndRange = Convert.ToInt32(rule.Split("-")[1].Split(" ")[0]);
                string Pattern = rule.Split(" ")[1].Trim();
        ...
        private int ValidateRulesNewPolicy(List<PasswordRule> passwordRules)
        {
            int validRules = 0;
            foreach (var passWordRule in passwordRules)
            {
                if (passWordRule.rule.EndRangeC >= passWordRule.password.Length)
                {
                    continue;
                }
                if ((passWordRule.rule.PatternC == passWordRule.password[passWordRule.rule.StartRangeC] && passWordRule.rule.PatternC != passWordRule.password[passWordRule.rule.EndRangeC]) || (passWordRule.rule.PatternC != passWordRule.password[passWordRule.rule.StartRangeC] && passWordRule.rule.PatternC == passWordRule.password[passWordRule.rule.EndRangeC]))
                {
                    validRules++;
                }
            }
        ...
        private int ValidateRules(List<PasswordRule> passwordRules)
        {
            int validRules = 0;
            foreach(var passWordRule in passwordRules)
            {
                int match = 0;
                for(int i = 0; i < passWordRule.password.Length; i++)
                {
                    if (passWordRule.password[i] == passWordRule.rule.Pattern.ToCharArray()[0])
                    {
                        match++;
                    }
                }
                if (match >= passWordRule.rule.StartRange && match <= passWordRule.rule.EndRange)
                {
                    validRules++;
                }
            }
...
public class Rule
{
    ...
    public char PatternC { 
        get {
            return Pattern.ToCharArray()[0];
        } 
    }
    public int StartRange { get; set; }
    public int EndRange { get; set; }
    public int StartRangeC { 
        get {
            return this.StartRange - 1;
        } 
    }
    public int EndRangeC
    {
        get {
            return this.EndRange - 1;
        }
    }
} 
