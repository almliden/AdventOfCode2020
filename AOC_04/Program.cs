...
namespace AOC_04
{
    class Program
    {
        ...
        {
            string realInput = @"eyr:2029 pid:157374862
...
iyr:2011 ecl:brn hgt:59in";
            ...
            List<string> allValidKeys = new List<string>
            {
                "byr",
                "iyr",
                "eyr",
    ...
    class Solver
    {
        public int Solve(string input, List<string> validKeys)
        {
            Regex regex = new Regex("\r\n\r\n");
            List<string> rawPassports = regex.Split(input).ToList();

            List<Dictionary<string, string>> passports = new List<Dictionary<string, string>>();

            Regex regex1 = new Regex(@"\s");
            foreach(string s in rawPassports)
            {
                List<string> keyValues = regex1.Split(s).ToList();
                Dictionary<string, string> passport = new Dictionary<string, string>();
                foreach(string part in keyValues)
                {
            ...
            return FindValid(passports, validKeys);
        }

        private int FindValid(List<Dictionary<string, string>> passports, List<string> validkeys)
        {
